...
const QuestionList = ({ items, onReorder, onEdit, OnDelete, }) => {
    ...
    return (<dnd_1.DragDropContext onDragEnd={onDragEnd}>
      <dnd_1.Droppable droppableId="chapters">
        {(provided) => (<div {...provided.droppableProps} ref={provided.innerRef}>
            ...
            {questions.sort((a, b) => a.position - b.position).map((question, index) => (<dnd_1.Draggable key={question.id} draggableId={question.id} index={index}>
                {(provided) => (<div className={(0, utils_1.cn)("flex items-center bg-sky-100 border-sky-200 text-sky-700 gap-x-2 border rounded-md mb-4 text-sm", question.isPublished &&
                    ...
                    <div className="ml-auto pr-2 flex items-center gap-x-2">
                      ...
                      <lucide_react_1.Delete onClick={() => { OnDelete(question.id); }} className="w-4 h-4 cursor-pointer hover:opacity-75 transition"/>
