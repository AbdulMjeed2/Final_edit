...
const lessons_list_1 = require("./lessons-list");
const formSchema = z.object({
    title: z.string().min(1),
});
const LessonsForm = ({ initialData, chapterId, courseId, }) => {
    ...
    const onSubmit = (values) => __awaiter(void 0, void 0, void 0, function* () {
        try {
            yield axios_1.default.post(`/api/courses/${courseId}/chapters/${chapterId}/lessons`, values);
            react_hot_toast_1.default.success("تم إنشاء الدرس");
            toggleCreating();
            form.setValue("title", "");
            router.refresh();
        }
    ...
    const onReorder = (updateData) => __awaiter(void 0, void 0, void 0, function* () {
        try {
            setIsUpdating(true);
            yield axios_1.default.put(`/api/courses/${courseId}/chapters/${chapterId}/lessons/reorder`, {
                list: updateData,
            });
            react_hot_toast_1.default.success("تم إعادة ترتيب الدروس");
            router.refresh();
        }
        catch (_a) {
            react_hot_toast_1.default.error("هناك شئ غير صحيح");
    ...
    return (<div className="relative mt-6 border bg-slate-100 rounded-md p-4">
      ...
      {isCreating && (<form_1.Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 mt-4">
            <form_1.FormField control={form.control} name="title" render={({ field }) => (<form_1.FormItem>
                  <form_1.FormControl>
                    <input_1.Input disabled={isSubmitting} placeholder="e.g. 'مقدمة للدرس'" {...field}/>
                  </form_1.FormControl>
                  <form_1.FormMessage />
                </form_1.FormItem>)}/>
      ...
      {!isCreating && (<div className={(0, utils_1.cn)("text-sm mt-2", !initialData.lessons.length && "text-slate-500 italic")}>
          {!initialData.lessons.length && "لا دروس"}
          <lessons_list_1.LessonsList onEdit={onEdit} onReorder={onReorder} items={initialData.lessons || []}/>
        </div>)}
...
exports.LessonsForm = LessonsForm;
