...
const TaskForm = ({ initialData, courseId }) => {
    ...
    const onSubmit = (values) => __awaiter(void 0, void 0, void 0, function* () {
        try {
            const response = yield axios_1.default.post(`/api/courses/${courseId}/task`, values);
            react_hot_toast_1.default.success("تم إنشاء المهمة");
            toggleCreating();
            router.refresh();
        }
        catch (error) {
    ...
    const onDelete = (id) => __awaiter(void 0, void 0, void 0, function* () {
        try {
            //  setIsDeleting(true);
            debugger;
            yield axios_1.default.delete(`/api/courses/${courseId}/task/${id}`);
            //deleted(id)
    ...
    return (<div className="relative mt-6 border bg-slate-100 rounded-md p-4">
      ...
      <div className="font-medium flex items-center justify-between">
        مهام الدورة
        {!(task) && (<button_1.Button onClick={toggleCreating} variant="ghost">
            {isCreating ? (<>إلغاء</>) : (<>
      ...
      {!isCreating && (<div className={(0, utils_1.cn)("text-sm mt-2", !task && "text-slate-500 italic")}>
          {!task ? ("لا توجد مهمة") : (<div className={(0, utils_1.cn)("flex justify-between items-center py-3 pl-3 gap-x-2 bg-slate-200 border-slate-200 border text-slate-700 rounded-md mb-4 text-sm")}>
                  <div> {task.title}</div>
                  <div className="ml-auto pr-2 flex items-center gap-x-2">
                    <badge_1.Badge className={(0, utils_1.cn)("bg-slate-500", task.isPublished && "bg-sky-700")}>
                      {task.isPublished ? "نشرت" : "مسودة"}
                    </badge_1.Badge>
                    <lucide_react_1.Delete onClick={() => onDelete(task.id)} className="w-4 h-4 cursor-pointer hover:opacity-75 transition"/>
                    {<lucide_react_1.Pencil onClick={() => onEdit(task.id)} className="w-4 h-4 cursor-pointer hover:opacity-75 transition"/>}
                  </div>
                </div>)}
        </div>)}
    </div>);
